...
public class SerialPortCommServer {
    ...
    public SerialPortCommServer(int headServerPort, int alertServerPort, int flexServerPort, int flexAuthServerPort) {
        logger.info("+++++++++++++++++++++++++++++++++++++ ,headServerPort:" + headServerPort + ",alertServerPort:" + alertServerPort + ",flexServerPort:" + flexServerPort + ",flexAuthServerPort:" + flexAuthServerPort);
        ...
        try {
            //启动云台控制服务
            serverHead = new Server(headServerPort, new HeadServerHandler());
            serverHead.setConnectionTimeoutMillis(10000);
            serverHead.setIdleTimeoutMillis(10000);
            serverHead.start();

            //启动报警检测服务
            serverAlert = new Server(alertServerPort, new AlertServerHandler());
            serverAlert.setConnectionTimeoutMillis(10000);
            serverAlert.setIdleTimeoutMillis(10000);
            serverAlert.start();

            //启动flex安全沙箱验证服务
            serverFlexAuth = new Server(flexAuthServerPort, new FlexAuthServerHandler());
            serverFlexAuth.start();

            //启动flex服务
            serverFlex = new Server(flexServerPort, new FlexServerHandler());
